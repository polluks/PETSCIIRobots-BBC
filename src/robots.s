; PETSCII Robots - BBC Micro Port
; Based on C64 version by David Murray 2020
; Ported to BBC Micro with ca65
...
MSG_TITLE:
    .byte 13,"PETSCII ROBOTS - BBC PORT",13
...
; ============================================================
MAIN_LOOP:
    ; Wait for IRQ tick (frame pacing)
ML_WAIT:
    LDA BGTIMER1
    BEQ ML_WAIT
    LDA #0
...
BIG_EXP:
    ...
    STA BIG_EXP_ACT
    SEI
    ; Flash border (simulated by changing screen)
    CLI
    LDX #1
BE_L:
    LDA UTYPE,X
...
; Tone channel 0: A = tone register byte (1CC0LLLL) or (0-HHHHHH)
SN_TONE0_LO:
    ORA #$80
    JSR SN_WRITE
    RTS
...
; Simple beep on channel 0
; A = frequency index (0-15, lower = higher)
SN_BEEP:
    PHA
    AND #$0F
    EOR #$0F
...
BEEP_TONES:
    .byte $1F, $2F, $4F, $6F, $8F, $AF, $CF, $EF
    .byte $0F, $3F, $5F, $7F, $9F, $BF, $DF, $FF
...
GL_DONE:
    ...
    ; Place enemies with BBC-tuned timers
    LDX #1
    LDA #UT_WALKER
    STA UTYPE,X
    LDA #27
...
; Carve room at (GEN_X,GEN_Y), 6x8 floor tiles
GL_ROOM:
    LDA #0
    STA TEMP_C
GR_Y2:
...
; GL_SET: set tile at (GEN_X,GEN_Y) = TILE
GL_SET:
    LDA GEN_X
    STA MAP_X
    LDA GEN_Y
...
TILE_CLRS:
    .byte 0, 0, TC_MAGENTA, TC_MAGENTA, TC_MAGENTA, TC_YELLOW, TC_YELLOW, TC_BLUE
    .byte 0, 0, 0, 0, 0, 0, TC_MAGENTA, 0
.repeat 240
    .byte 0
.endrepeat
